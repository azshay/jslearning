...
const Slider = (props) => {
	...
	function changeSlide(index) {
		setSlide((slide) => slide + index); // ЕСЛИ МЫ ПЕРЕДАЕМ НЕ CALLBACK ФУНКЦИЮ, ТО МЫ ПОЛУЧАЕМ ДВА АСИНХРОННЫХ КОДА, ЧТО МОЖЕТ ПРИВЕСТИ К ОШИБКЕ
		// setSlide(slide + index); // 1
		// setSlide(slide + index); // 1, т.к. опора была на прошлый STATE
	}
